...
class PENet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        for i, (skip_channels, in_channels, mid_channels, out_channels) in enumerate(
            decoder_config
        ):
            is_last_decoder = i == len(decoder_config) - 1
            decoder = PENetDecoder(
                skip_channels,
                in_channels,
                mid_channels,
                out_channels,
        ...
        for m in self.modules():
            if (
                ...
                if hasattr(m, "bias") and m.bias is not None:
                    if (
                        focal_pi is not None
                        and hasattr(m, "is_output_head")
                        and m.is_output_head
                    ):
    ...
    def fine_tuning_parameters(self, fine_tuning_boundary, fine_tuning_lr=0.0):
        ...
        optimizer_parameters = [
            {
                "params": gen_params(fine_tuning_boundary, fine_tuning=True),
                "lr": fine_tuning_lr,
            },
